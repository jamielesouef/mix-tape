...
- `isAVPlayerNative(container:videoCodec:audioCodec:) -> Bool`, a pure function in Domain, per the §8 fixture rule: container ∈ `mp4, m4v, mov` and video codec ∈ `h264, hevc` and audio codec ∈ `aac, mp3, alac, ac3, eac3` → `true`; anything else → `false`.
...
- [ ] `MixtapeDomainTests` builds and every fixture-table case in `isAVPlayerNative` passes: `(mp4, h264, aac)` → `true`; `(mkv, h264, aac)` → `false`; `(mkv, hevc, dts)` → `false`; `(mov, hevc, ac3)` → `true`; `(webm, vp9, opus)` → `false`; `(mp4, h264, dts)` → `false`; `(mp4, av1, aac)` → `false`.
