...
| 2026-09-07 | Player routing (`.directAVPlayer` vs `.directVLC`) for a direct-stream-only source is decided by AC22c's live measurement, recorded here once run, not asserted in advance — **only applicable if the §4 spike finds a reachable case**. **[Fill in at implementation: which controller played the reachable case, and whether `isAVPlayerNative`'s input needed to change from the source's original container to the remux target container.]** | Assuming `AVPlayer` handles the remux and shipping without a live check, on the strength of the URL being "the same kind of GET" as `static=true`. | 018's decision log already measured a *different* remux shape (audio, `transcodingContainer=mp4`, `transcodingProtocol=http`) failing on `AVPlayer` with `Accept-Ranges: none` and no `Content-Length` — `-12939`, unplayable, not merely un-seekable. Whether the video direct-stream URL this slice builds shares that non-seekable shape is a server-response question this slice's own pre-flight has not yet answered, and shipping a routing decision on an unmeasured assumption is exactly the failure 018 §5's evidence paragraph exists to prevent repeating. |
...
- [ ] Any `SPEC-DECISIONS.md` amendment this slice needs is appended, not edited in place
