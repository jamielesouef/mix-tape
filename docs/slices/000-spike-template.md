...
Bad: "Investigate the device profile." Good: "Does a single permissive device profile
covering VLC's range keep an mkv/hevc/dts source off the server's transcoder, or does
Jellyfin still transcode when the profile allows a container AVPlayer cannot open?"
