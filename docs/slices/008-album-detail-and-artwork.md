...
|---|---|---|---|
| 2026-09-01 | **Artwork is served by its own `FileMiddleware` rooted at the artwork cache directory and mounted at `urlBasePath: "/artwork"`, alongside a second instance for audio** — the plan's layout, now proven rather than assumed (**S001**) | The single-root fallback in S001 §5, with music bind-mounted beneath the cache directory and one `FileMiddleware` at `/`; a hand-written artwork route reading the file itself | S001 measured it on Hummingbird 2.26.0: two instances on two roots coexist, and `FileMiddleware` calls `next` **before** looking for a file, so it can never shadow a route no matter the registration order. The fallback would have weakened ladder **L1**'s seam by forcing `MIXTAPE_MUSIC_DIR` inside `MIXTAPE_CACHE_DIR`; it is not needed. The `urlBasePath` spelling is exact — leading slash, no trailing slash — and the prefix is stripped before the root is joined, so `GET /artwork/{albumID}` resolves to `<artworkRoot>/{albumID}` |
