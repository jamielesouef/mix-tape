...
- Playback reporting (start/progress/stopped) and resume — deferred to **008**. `VideoPlaybackService` plays, pauses, seeks and stops locally; nothing is told to the server yet.
...
- `§12.6` — an `.mp4/h264/aac` movie plays via `.directAVPlayer` with no transcode session in the dashboard. Satisfied by the Avatar file and the direct-play branch above.
- `§12.8` — a source the device profile rejects plays via `.transcodeHLS` and the dashboard shows a transcode. Satisfied by the DEBUG launch argument forcing F1 through the restrictive profile, per decision 14's finding that this criterion is a fixture problem, not a media-sourcing one.
...
- [ ] `MixtapeServicesTests`, tagged `.service`: `VideoPlaybackService` status transitions (`.idle` → `.preparing` → `.playing` → `.paused` → `.failed`) against a stub `VideoPlayerControlling`.

Acceptance (server-observable, against `http://localhost:8096`):
- [ ] AC6: play the Avatar `mp4`/h264 movie; `GET /Sessions` shows no `TranscodingInfo` on this device's session, and the Jellyfin dashboard lists no active transcode. (`NowPlayingItem` and `PlayMethod` appear on the session only after the start report slice 008 wires, so that half of the dashboard check is 008's, not this slice's.)
- [ ] AC8: relaunch with the `-mixtape-force-transcode` launch argument, play the F1 `mkv` movie; `GET /Sessions` shows `TranscodingInfo` present on this device's session. `TranscodingInfo` is attached by the transcoder keyed on `DeviceId`, so it needs no start report.
