...
| 2026-09-01 | **Slice 001 review** | **The SPDX line-1 rule was unsatisfiable for two files.** `CLAUDE.md`, plan §7 and slice 002 all require `SPDX-License-Identifier` on line 1 of every source file, with `check-spdx.sh` asserting it. Swift parses `// swift-tools-version:` positionally and will not build a `Package.swift` without it on line 1, so `Shared/Package.swift` and `Server/Package.swift` could not satisfy both rules. Slice 002 would have hit this head-on | A single filename-keyed exemption: in a `Package.swift` the tools-version line is line 1 and SPDX is line 2, and `check-spdx.sh` branches on the filename to assert both. Narrow enough to stay machine-checkable — an exemption a script cannot express is a hole. Written into `CLAUDE.md`'s Licensing and Conventions sections and into 002's scope, acceptance criteria and decision log | 002 |
| 2026-09-01 | **Slice 001 review** | **A build fix was recorded only as a decision row.** `ENABLE_ENHANCED_SECURITY = NO` was set on the `MixTape` target to resolve an `arm64e`-versus-`arm64` module mismatch against the local `Shared` package. The fix is correct, but it silently weakens the macOS security posture and a decision row is where that gets forgotten | Promoted to ladder **L13** with the seam named — how `Shared` is consumed by the app. Ladder count 12 to 13 | 001 |
...
**Version ladders (13).** A ladder is only valid if v1 sits behind the seam v2 replaces — every seam is named:
...
| L13 | **macOS ships with `ENABLE_ENHANCED_SECURITY = NO`.** Xcode 27 sets it on by default, which forces the app to `arm64e` while the local `Shared` SwiftPM package builds plain `arm64` — the macOS build then fails to find the `Shared` module. Turning it off was reviewed and judged the correct fix; forcing `arm64e` on the package with `unsafeFlags` was the only alternative and is worse | **How `Shared` is consumed by the app.** v2 makes `Shared` an Xcode-native target, or a prebuilt xcframework covering `arm64e`, either of which removes the architecture mismatch and lets the setting go back on. Recorded as a ladder rather than a decision row because the crude rung silently weakens a security posture, and a decision row is where that gets rediscovered at release | 001, and whichever slice hardens the macOS build before release |
