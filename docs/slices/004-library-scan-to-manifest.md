...
- `Server/Tests/ServerTests/TestTags.swift` — the `Tag` extension every slice from here on already assumes exists. `CLAUDE.md` requires suites tagged by layer, and **no slice has ever created the tags**; slice 001 named the convention and shipped empty test targets, so `@Suite(.tags(.domain))` does not compile anywhere in this repository today. Define all four — `.domain`, `.useCase`, `.service`, `.repository` — even though the server side never uses `.service`, so the vocabulary is identical on both sides and nobody has to remember which half of it is legal where. This is the first slice that writes a server suite, so it is the first that would trip on the gap
...
- [ ] **Every suite in `Server/Tests/ServerTests/` carries a layer tag, and `swift test --filter` on a tag selects only that layer.** Run it — a tag that compiles but selects nothing is the same class of fault as a green suite that executed no tests.
...
- **The tags themselves do not exist yet and this slice creates them** — `Server/Tests/ServerTests/TestTags.swift`, defining all four layer tags as a `Tag` extension. Every slice from 004 to 011 names a tag in this same line of its own Section 8, and none of them creates one. Writing a tagged suite before the extension exists is a compile error, not a lint warning, so this is the first line of test code the slice writes rather than something to tidy at the end.
