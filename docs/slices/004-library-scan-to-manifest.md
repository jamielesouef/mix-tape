...
- One `ffprobe -v quiet -print_format json -show_format -show_streams` `Process` per file, in a bounded `withTaskGroup` capped at `ProcessInfo.processInfo.activeProcessorCount`
...
- [ ] **S002** — spike, part 2. If `ffprobe` was not discoverable from a `Process`, add `MIXTAPE_FFPROBE_PATH` to `ServerConfiguration` now. Two lines here, a debugging session later.
