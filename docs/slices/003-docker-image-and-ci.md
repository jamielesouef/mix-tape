...
- [x] **`server.yml` compiles `Shared` on Linux, and `import CoreGraphics` in a `Shared` file fails that job.** Run deliberately on PR [#1](https://github.com/jamielesouef/mix-tape/pull/1), `ci-proof/coregraphics`, then closed unmerged. The Linux job failed with `Shared/Sources/Shared/VersionResponseDTO.swift:9:8: error: no such module 'CoreGraphics'` ([run 33477197249](https://github.com/jamielesouef/mix-tape/actions/runs/33477197249)); `Build and push the image` was correctly **skipped** on the failure, and `Repository checks` passed, so the failure is the one intended and not a knock-on. **The pairing is the whole argument:** the same commit passed `app.yml` on macOS, because CoreGraphics exists there. That is precisely why slice 001's grep was never sufficient on its own, and why `1.linux` was written as "confirmed by inspection, binding proof in CI". It is binding from this run onward.
- [x] **The image reaches GHCR.** `Build and push the image` succeeded on the `main` run and the log records `pushing manifest for ghcr.io/jamielesouef/mix-tape:d661bed…@sha256:13dc4de3…`, tagged `:latest` and `:<sha>`, `platforms: linux/amd64`.
...
**The local run used the selected Xcode 27, not the pinned 26.6 — and CI has now closed that gap.** Xcode 26.6 builds this project on both destinations locally but cannot drive a simulator here, because `CoreSimulator` is owned by whichever Xcode is `xcode-select`ed and a per-command `DEVELOPER_DIR` leaves `xcodebuild -showdestinations` showing only a placeholder. Switching globally needs `sudo` and changes the owner's machine for one check. The runner has no such problem, and the [first `app.yml` run](https://github.com/jamielesouef/mix-tape/actions/runs/33476417126) proves the pinned path end to end: `Xcode 26.6`, `Build version 17F113`, resolver output `Using simulator: iPhone 17 Pro`, both suites started — `VersionResponseDTOTests` and `VersionScreenUITests` — and `** TEST SUCCEEDED **`.
...
| 2026-09-01 | **`DEVELOPMENT_TEAM` is removed from the project file and supplied by a gitignored `App/Local.xcconfig`, included optionally by a committed `App/Signing.xcconfig`** | Leave the identifier in the project, since it is an identifier and not a credential; remove it and pass `DEVELOPMENT_TEAM=…` on the command line for macOS builds; rewrite history to erase it | Making the repository public put the owner's Apple team identifier on the internet in six places. Not a credential — nothing signs with it without the certificates — but it identifies a person, and a public project has no reason to carry one. **Removing it outright broke slice 001's macOS acceptance criterion**: the app's entitlements require a development certificate, and the build fails with "has entitlements that require signing with a development certificate". A command-line variable was rejected because the owner develops in Xcode, and Xcode writes the team straight back into the project file the moment it is set in the UI — the identifier would return inside a diff nobody expects to have to review. The optional include form (`#include?`) is what keeps it warning-free: a clone with no `Local.xcconfig` builds for the simulator cleanly, which is all CI does. **Both halves proven** — without the file the simulator build succeeds; with it the macOS build succeeds again |
| 2026-09-01 | **The identifier stays in the published git history, and that is an acceptance rather than an oversight** | Rewrite history with `git filter-repo` and force-push | It is in commit `bf1c14e`, which is public. Erasing it means rewriting every commit after it, force-pushing over a branch whose protection explicitly forbids force pushes, and invalidating every sha this project's own documents cite as evidence — the decision logs, the Drift Log rows and the spike results all reference commits by hash. **What is being protected does not justify that**: a team identifier appears in the receipt of every app its owner has ever shipped, and GitHub may already have cached the old objects regardless. Removing it going forward is proportionate; rewriting history is not. Written down so nobody later assumes the history was cleaned |
...
| 2026-09-01 | **Both workflows also carry a `workflow_dispatch` trigger** | Leave `push` and `pull_request` only | **Added on a reason that turned out to be wrong, and kept on a different one — recording both, because the wrong reason is the more useful half.** `gh run list` came back empty straight after the first push, and that was read as GitHub declining to evaluate a path-filtered `push` trigger on branch creation. It was not: the check was simply too early, and both workflows had in fact queued and were running by the next look. So the stated justification was false. The trigger stays because it is independently worth having — re-running CI against unchanged code after GitHub rotates a runner image otherwise needs a throwaway commit — but it was added to fix a problem that did not exist. **The lesson is the row: an empty result from an asynchronous system is not evidence of anything until it has been given time to be non-empty.** |
...
| 2026-09-01 | **The repository went public later the same day, and `main` is now a protected branch** | Stay private and pay for GitHub Pro to get protection; stay private and settle for a local `pre-push` hook | Asking for direct pushes to `main` to be blocked ran straight into a billing wall: branch protection and rulesets both return `403 Upgrade to GitHub Pro or make this repository public`. A local hook was rejected as enforcement that is not enforcement — it lives on one machine, is absent from the repository, and `--no-verify` steps over it. **Owner chose public**, which the handoff wanted from the first public commit anyway and which slice 002's licence split exists to make safe. The history was scanned for credentials before the switch and carries none. `main` now requires a pull request, blocks force pushes and deletions, requires linear history, and applies to administrators — **proven by attempting a direct push and being refused with `GH006: Protected branch update failed`**, not by reading the settings page |
...
- [x] Acceptance criteria met — all of them. The three that needed CI closed on the first push to `jamielesouef/mix-tape` and two deliberate throwaway pull requests
- [x] Tests passing, in a target that exists — **the targets existed and could not run**, which is why this box needed the fix rather than the tick. `PRODUCT_NAME` added to both, the two tests slice 001's Section 8 specified written, and both suites confirmed running on the pinned Xcode 26.6 in CI
