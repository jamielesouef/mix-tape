...
Review the whole directory, not slice by slice. Cross-document faults are invisible from
inside any single slice:

- [ ] Every `next_slice` has a matching `previous_slice` on the target. Both directions.
- [ ] Delivery order in the checklist matches the linked-list order.
- [ ] Every `depends_on` id exists, and no cycle.
- [ ] Every requirement in `SPEC-DECISIONS.md` and the engineering doc's scope list is in
      some slice's `covers:`, or has a fork row saying why not.
- [ ] No slice's `covers:` claims a requirement its acceptance criteria cannot demonstrate.
- [ ] Every fork has a decision row naming what was rejected.

The original convention ran a `slice-set-review` skill here. It is not installed on this
machine, so the checklist above is the manual equivalent — the same faults, checked by hand.
