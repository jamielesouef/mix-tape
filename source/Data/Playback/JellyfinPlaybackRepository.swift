...
nonisolated struct JellyfinPlaybackRepository: PlaybackRepositoryProtocol {
    ...
    /// Containers the server may hand over untouched; anything else is transcoded.
    private static let directPlayContainers = "flac,alac,m4a,mp3,aac,wav,aiff"
    ...
    func audioStream(track: MediaItem, session: UserSession, playSessionID: String) -> AudioStream {
        guard isNativeAudioContainer(track.container) else {
            let container = track.container ?? "?"
            AppLogger.playback
                .info("audio HLS fallback fired for track \(track.id) (container \(container))")

            return AudioStream(
                url: hlsFallbackURL(track: track, session: session, playSessionID: playSessionID),
                playMethod: .transcode
            )
        }

        var components = URLComponents(
            url: session.serverURL.appending(path: "/Audio/\(track.id)/universal"),
        ...
        return AudioStream(url: components?.url ?? session.serverURL, playMethod: .directPlay)
    }
    ...
    private func hlsFallbackURL(
        track: MediaItem,
        session: UserSession,
        playSessionID: String
    ) -> URL {
        var components = URLComponents(
            url: session.serverURL.appending(path: "/Audio/\(track.id)/main.m3u8"),
            resolvingAgainstBaseURL: false
        )
        components?.queryItems = [
