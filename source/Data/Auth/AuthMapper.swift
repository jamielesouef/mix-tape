...
enum AuthMapper {
    ...
    static func userSession(
        from dto: AuthenticationResultDTO,
        serverURL: URL,
        deviceID: String
    ) throws -> UserSession {
        guard let token = dto.accessToken, let userID = dto.user?.id else {
    ...
    static func handshake(from dto: QuickConnectResultDTO) throws -> QuickConnectHandshake {
        guard let secret = dto.secret, let code = dto.code else {
            throw MixtapeError.decoding
        }
