...
final class ImageService {
    ...
    func imageURL(for item: MediaItem, kind: ImageKind, maxHeight: Int) -> URL? {
        ...
        // Tracks rarely carry art of their own, so the album's primary image stands in.
        let standsInForAlbum = kind == .primary
            && item.kind == .audio
            && item.primaryImageTag == nil

        if standsInForAlbum, let albumID = item.albumID {
            return builder.url(
                itemID: albumID,
