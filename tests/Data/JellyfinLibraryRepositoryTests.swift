...
struct JellyfinLibraryRepositoryTests {
    ...
    @Test
    func `libraries map collection types and send the user id and token`() async throws {
        ...
        #expect(stub.lastRequest?
            ...
            .contains(#"Token="tok-1""#) == true)
    }

    @Test
    func `unsupported collection types map to unsupported`() async throws {
        stub.respond(
            status: 200,
            json: #"""
            {"Items": [
                {"Id": "b", "Name": "Books", "Type": "CollectionFolder",
                 "CollectionType": "books"},
                {"Id": "f", "Name": "Folder", "Type": "CollectionFolder"}
            ], "TotalRecordCount": 2}
            """#
        )
        let libraries = try await repository.libraries(session: session)
    ...
    @Test
    func `album page maps album artist and year`() async throws {
    ...
    @Test
    func `tracks carry the album id and album art and are sorted by disc and track`() async throws {
        ...
        #expect(sentQuery["parentId"] == "album-1")
        #expect(sentQuery["includeItemTypes"] == "Audio")
        #expect(sentQuery["sortBy"] == "ParentIndexNumber,IndexNumber,SortName")
    }

    @Test
    func `item detail maps the resume position and keeps fields`() async throws {
        stub.respond(
            status: 200,
            json: #"""
            {"Id": "album-1", "Name": "Even In Arcadia", "Type": "MusicAlbum",
    ...
    @Test
    func `unknown item type on detail is a decoding error`() async {
    ...
    @Test
    func `a 401 on a library call is session expiry`() async {
