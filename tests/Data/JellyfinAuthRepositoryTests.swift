...
struct JellyfinAuthRepositoryTests {
    ...
    @Test
    func `server identity maps the public info`() async throws {
        stub.respond(
            status: 200,
            body: Data(
                #"{"Id":"srv","ServerName":"mixtape","Version":"10.11.11","LocalAddress":"http://x"}"#
                    .utf8
            )
        )
    ...
    @Test
    func `authenticate sends username and pw and maps the session`() async throws {
    ...
    @Test
    func `authenticate without A token is A decoding error`() async {
    ...
    func `quick connect enabled decodes A bare boolean`(json: String, expected: Bool) async throws {
        ...
        #expect(stub.lastRequest?
            .value(forHTTPHeaderField: "Authorization")?
            .hasPrefix("MediaBrowser ") == true)
    }

    @Test
    func `initiate is A post carrying the device id`() async throws {
        stub.respond(
            status: 200,
        ...
        #expect(stub.lastRequest?
            .value(forHTTPHeaderField: "Authorization")?
            .contains(#"DeviceId="device-1""#) == true)
    }

    @Test
    func `quick connect state reads authenticated`() async throws {
        stub.respond(
            status: 200,
            body: Data(#"{"Authenticated":true,"Secret":"s3cret","Code":"426349"}"#.utf8)
        )
    ...
    @Test
    func `authenticate with quick connect sends the secret`() async throws {
        stub.respond(status: 200, body: Data(authResult.utf8))
