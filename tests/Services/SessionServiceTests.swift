...
struct SessionServiceTests {
    ...
    @Test
    func `restore lands signed in when A stored session exists`() async {
    ...
    @Test
    func `restore lands signed out without A stored session`() async {
    ...
    @Test
    func `restore lands signed out when the store fails`() async {
        let service =
            makeService(store: MockSessionStore(failure: MixtapeError.transport("keychain")))
        await service.restore()
        #expect(service.state == .signedOut)
    ...
    @Test
    func `validate server sets the identity or the error`() async {
        let service = makeService()
        await service.validateServer(urlText: "localhost:8096")
        #expect(service.serverIdentity == server)
        #expect(service.error == nil)

        let failing = makeService(repository: MockAuthRepository(serverIdentityResult: { _ in
    ...
    @Test
    func `clear server returns to server entry with nothing carried over`() async {
        let service = makeService(repository: MockAuthRepository(serverIdentityResult: { _ in
            throw MixtapeError.notAJellyfinServer
        }))
    ...
    @Test
    func `sign in transitions to signed in`() async {
    ...
    @Test
    func `wrong password keeps the server and reports invalid credentials`() async {
        let repository = MockAuthRepository(authenticateResult: { _, _, _ in
    ...
    @Test
    func `session expiry clears the store and returns to signed out`() async {
    ...
    @Test
    func `sign out clears everything`() async {
        let store = MockSessionStore(session: session)
        let service = makeService(store: store, serverIdentity: server)
        await service.restore()
        service.signOut()
    ...
    @Test
    func `quick connect shows the code then signs in on approval`() async {
    ...
    @Test
    func `quick connect expires after five minutes`() async {
    ...
    @Test
    func `cancel stops polling and returns to idle`() async {
    ...
    @Test
    func `quick connect unavailable is reported on the quick connect state`() async {
    ...
    @Test
    func `poll failure is reported and stops polling`() async {
        let repository = MockAuthRepository(quickConnectStateResult: { _, _ in
            throw MixtapeError.transport("404")
        })
        let clock = StubClock()
        let service = makeService(repository: repository, clock: clock, serverIdentity: server)
        await service.startQuickConnect()
