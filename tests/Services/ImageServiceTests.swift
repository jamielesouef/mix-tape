...
struct ImageServiceTests {
    ...
    @Test
    func `a track without its own art falls back to the album`() throws {
        let track = MockLibraryRepository.sampleTracks[0]

        let url = try #require(service.imageURL(for: track, kind: .primary, maxHeight: 300))
        let components = try #require(URLComponents(url: url, resolvingAgainstBaseURL: false))
        let query = try #require(components.queryItems)

        #expect(query.contains(URLQueryItem(name: "itemId", value: track.albumID)))
        #expect(query.contains(URLQueryItem(name: "tag", value: track.parentPrimaryImageTag)))
    }

    @Test
    func `an item without a tag has no url and no image`() async {
    ...
    @Test
    func `no session means no url`() {
        let signedOut = MockImageService.make(sessionService: MockSessionService.signedOut())

        #expect(signedOut.imageURL(
            for: MockLibraryRepository.sampleAlbums[0],
            kind: .primary,
            maxHeight: 300
        ) == nil)
    }
    ...
    @Test
    func `AC23c two concurrent requests for the same url are coalesced into one network call`(
    ) async throws {
        ...
        let networked = ImageService(
            builder: MockImageURLBuilder(),
            sessionService: MockSessionService.signedIn(),
            urlSession: StubImageURLProtocol.session
        )
    ...
    @Test
    func `AC23c a non-2xx response yields nil instead of decoding the body`() async throws {
    ...
    @Test
    func `AC23c cache cost is the decoded pixel size, not the compressed byte count`() throws {
        let imageData = Self.fixturePNGData()
