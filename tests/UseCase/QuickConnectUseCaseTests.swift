...
struct QuickConnectUseCaseTests {
    ...
    @Test
    func `start returns the handshake when enabled`() async throws {
        let handshake =
            try await StartQuickConnectUseCase(repository: MockAuthRepository())(server: server)
        #expect(handshake == QuickConnectHandshake(secret: "secret", code: "123456"))
    }

    @Test
    func `start throws unavailable when the server says disabled`() async {
    ...
    @Test
    func `poll returns nil while unapproved and exchanges once approved`() async throws {
        let store = MockSessionStore()
        let pending = MockAuthRepository(quickConnectStateResult: { _, _ in false })
        let pendingResult = try await PollQuickConnectUseCase(repository: pending, store: store)(
            secret: "secret",
            server: server
        )
        ...
        let session = try await PollQuickConnectUseCase(repository: approved, store: store)(
            secret: "secret",
            server: server
        )
    ...
    func `poll propagates failures`(error: MixtapeError) async {
        ...
        await #expect(throws: error) {
            try await PollQuickConnectUseCase(repository: repository, store: MockSessionStore())(
                secret: "secret",
                server: server
            )
        }
