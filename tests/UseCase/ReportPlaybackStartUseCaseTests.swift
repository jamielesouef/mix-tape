...
struct ReportPlaybackStartUseCaseTests {
    ...
    @Test
    func `the report reaches the repository`() async {
        let recorder = Recorder()
        let repository = MockPlaybackRepository(reportStartResult: { report, _ in
            recorder.append("\(report.itemID) \(report.playMethod)")
        })
    ...
    @Test
    func `a repository failure is swallowed`() async {
        let repository = MockPlaybackRepository(reportStartResult: { _, _ in
