...
struct LibraryUseCaseTests {
    ...
    @Test
    func `fetch libraries filters unsupported`() async throws {
        let libraries =
            try await FetchLibrariesUseCase(repository: MockLibraryRepository())(session: session)
        #expect(libraries.map(\.kind) == [.music])
    }

    @Test
    func `fetch library items passes library kind and page through`() async throws {
        ...
        let page = try await FetchLibraryItemsUseCase(repository: repository)(
            libraryID: "lib",
            kind: .musicAlbum,
            page: PageRequest(startIndex: 60, limit: 60),
            session: session
        )
    ...
    @Test
    func `fetch item detail returns the item`() async throws {
        let item = try await FetchItemDetailUseCase(repository: MockLibraryRepository())(
            id: "album-1",
            session: session
        )
    ...
    @Test
    func `fetch album tracks passes the album id`() async throws {
        let recorder = Recorder()
        let repository = MockLibraryRepository(tracksResult: { id, _ in
            recorder.append(id)
            return MockLibraryRepository.sampleTracks
        })
        let tracks = try await FetchAlbumTracksUseCase(repository: repository)(
            albumID: "album-1",
            session: session
        )
    ...
    func `repository errors propagate unchanged`(error: MixtapeError) async {
        ...
        await #expect(throws: error) {
            try await FetchLibrariesUseCase(repository: repository)(session: session)
        }
        await #expect(throws: error) {
