...
struct ReportPlaybackProgressStoppedUseCaseTests {
    ...
    @Test
    func `progress reaches the repository`() async {
        let recorder = Recorder()
        let repository = MockPlaybackRepository(reportProgressResult: { report, _ in
            recorder.append("progress \(report.position.components.seconds)")
        })
    ...
    @Test
    func `progress failure is swallowed`() async {
        let repository = MockPlaybackRepository(reportProgressResult: { _, _ in
            throw MixtapeError.transport("503")
        })
        await ReportPlaybackProgressUseCase(repository: repository)(report, session: session)
    }

    @Test
    func `stopped reaches the repository`() async {
        let recorder = Recorder()
        let repository = MockPlaybackRepository(reportStoppedResult: { report, _ in
            recorder.append("stopped \(report.itemID)")
        })
    ...
    @Test
    func `stopped failure is swallowed`() async {
        let repository = MockPlaybackRepository(reportStoppedResult: { _, _ in
            throw MixtapeError.transport("503")
        })
        await ReportPlaybackStoppedUseCase(repository: repository)(report, session: session)
    }
